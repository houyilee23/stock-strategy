...
def render_index_html(accounts_data: dict, view_mode: str = "admin",
                       universe_by_category: dict = None) -> str:
    """產 /index.html (admin) 或 /<view_mode>.html (user)。純讀取版（已移除 sync）。

    view_mode:
      - "admin" → 完整檢視（全部 tab）
      - "Katie" → 使用者檢視（只看 Katie list + universe pool）

    universe_by_category: {category_name: [row_dicts]}
      若提供且 current 是 universe → 顯示折疊卡片（依類別 group）
      預設全部 collapse (closed)，使用者按需要展開。
    """
    is_admin = view_mode == "admin"
    if is_admin:
        visible_accounts = list(accounts_data.keys())
        page_title = "📊 台股策略訊號（管理員）"
        page_subtitle = "Takeshi / Katie / universe 三個清單檢視"
    else:
        visible_accounts = [view_mode]
        if "universe" in accounts_data:
            visible_accounts.append("universe")
        page_title = f"📊 {view_mode} 的投資清單"
        page_subtitle = "我的清單 + 股票池檢視"
...
<div x-data='{{
  ...
  filtered() {{
    ...
    if (q) {{
      rows = rows.filter(r => r.sid.includes(q) || (r.name||"").toLowerCase().includes(q));
    }}
    if (this.sortKey) {{
...
"""
    ...
    summaries = {a: len(r) for a, r in accounts_data.items()}
    for acc, n in summaries.items():
...
<!-- 股票池 tab：折疊卡片，依類別 group，預設全部 collapse -->
<div x-show="current === 'universe'">
  <small style="color:var(--pico-muted-color); display:block; margin-bottom:0.6rem;">
    依類別分組，點類別標題展開；搜尋時自動展開有結果的類別。
  </small>
  <template x-for="cat in Object.keys(universeByCategory)" :key="cat">
    <details :open="shouldOpen(cat) && filteredCategory(cat).length > 0"
             x-show="categoryHasMatch(cat)" class="cat-card">
      <summary>
...
function cellTier(t) { return `<span class="tier-${t}">${t}</span>`; }
function cellRsi(r) {
  const v = parseInt(r);
  if (isNaN(v)) return r;
...
<p><strong>Tier</strong>：S/A/B/C 信心遞減，D=邊界（紙上交易），F=不建議</p>
<p><strong>趨勢與 RSI</strong>：▲ 多 / ▼ 空，RSI ≥75 過熱、≤30 過冷</p>
<p><strong>在倉狀態</strong>：✅ 真實持有（從 trades_帳戶.csv 推算）</p>
</div>
</details>
...
"""
    return html_head(page_title.split("（")[0]) + body + html_tail()
...
def main():
    ...
    # 建類別 map：依 watchlists.yaml 註解產 universe 折疊卡片群組
    sid_to_cat = _build_category_map_from_watchlists()
    universe_rows = accounts_data.get("universe", [])
    universe_by_category: dict = {}
    for r in universe_rows:
        cat = sid_to_cat.get(r["sid"], "未分類")
        universe_by_category.setdefault(cat, []).append(r)

    # 1. 寫首頁（admin / Takeshi 完整檢視）
    print("產生首頁 /index.html (admin) ...")
    idx_html = render_index_html(accounts_data, view_mode="admin",
                                  universe_by_category=universe_by_category)
